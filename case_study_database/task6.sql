...
and not (year(hd.ngay_lam_hop_dong) = 2021 and quarter(hd.ngay_lam_hop_dong) in (1,2))
...
having so_luong = (select sum(hdct1.so_luong) as sum  from hop_dong_chi_tiet hdct1
...
select dvdk.*, count(hdct.ma_hop_dong) as so_luong
from dich_vu_di_kem dvdk
join hop_dong_chi_tiet hdct on hdct.ma_dich_vu_di_kem = dvdk.ma_dich_vu_di_kem
group by hdct.ma_dich_vu_di_kem
having so_luong = 1 
